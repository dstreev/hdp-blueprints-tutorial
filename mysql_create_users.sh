...
mysql ${CONN} CREATE USER '${AMBARI_USER}'@'%' IDENTIFIED BY '${AMBARI_USER_PASSWORD}'
mysql ${CONN} GRANT ALL PRIVILEGES ON ${AMBARI_DB}.* TO '${AMBARI_USER}'@'%';
...
mysql ${CONN} CREATE USER '${HIVE_USER}'@'%' IDENTIFIED BY '${HIVE_USER_PASSWORD}'
mysql ${CONN} GRANT ALL PRIVILEGES ON ${HIVE_DB}.* TO '${HIVE_USER}'@'%';
...
mysql ${CONN} CREATE USER '${OOZIE_USER}'@'%' IDENTIFIED BY '${OOZIE_USER_PASSWORD}'
mysql ${CONN} GRANT ALL PRIVILEGES ON ${OOZIE_DB}.* TO '${OOZIE_USER}'@'%';
...
mysql ${CONN} CREATE USER '${RANGER_USER}'@'%' IDENTIFIED BY '${RANGER_USER_PASSWORD}'
mysql ${CONN} GRANT ALL PRIVILEGES ON ${RANGER_DB}.* TO '${RANGER_USER}'@'%';
...
mysql ${CONN} CREATE USER '${RANGER_AUDIT_USER}'@'%' IDENTIFIED BY '${RANGER_AUDIT_USER_PASSWORD}'
mysql ${CONN} GRANT ALL PRIVILEGES ON ${RANGER_AUDIT_DB}.* TO '${RANGER_AUDIT_USER}'@'%';
for i in $(eval echo ${RANGER_HOSTS}); do
